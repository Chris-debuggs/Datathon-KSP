...
// TODO: Replace with real apiFetch() when Chris confirms /api/chat endpoint:
...
// Ticket 4.2: now uses apiFetch wrapper for 429/504 handling
const pollJobStatus = (jobId, attempt = 1, onComplete, onError) => {
  ...
  setTimeout(async () => {
    try {
      // Ticket 4.2: apiFetch catches 429/504 automatically
...
const Chat = () => {
  ...
  const handleAsyncJob = (jobId) => {
    ...
    pollJobStatus(jobId, 1,
      (graphData) => {
        setIsPolling(false);
        setPollingMessage('');
        // Navigate to graph page with real nodes + edges from Stratus
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setIsLoading(false);
      // Ticket 4.2: use isRateLimitError from apiFetch wrapper
      if (isRateLimitError(err)) {
        setSystemBusy(true);
      }
    }
      ...
      <div style={s.layout}>
        ...
        <div style={s.chatMain}>
          ...
          <div ref={messagesAreaRef} style={s.messages}>
            ...
            {/* Ticket 4.2: exact system busy message per spec */}
            {systemBusy && (
                        ...
                        <div style={s.xaiRow}>
                          {msg.source_nodes.map((node, i) => (
                            <span
                              ...
                              style={s.xaiTag}
                              onClick={() => {
                                // Navigate to FIR detail using React Router
                                if (node.CrimeNo) {
                                  navigate(`/fir/${node.CrimeNo}`);
                                } else if (node.CaseMasterID) {
                                  navigate(`/fir/${node.CaseMasterID}`);
                                }
                              }}
